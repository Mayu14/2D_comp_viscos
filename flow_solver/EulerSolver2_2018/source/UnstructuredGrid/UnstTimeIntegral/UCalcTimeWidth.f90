...
!	Other:
!***********************************/
subroutine UCalcTimeWidth(UG,CC)
use StructVar_Mod
use LoopVar_Mod
...
!$omp parallel num_threads(CoreNumberOfCPU),shared(UG,CC),firstprivate(iCell,CTW)
!$omp do
...
!$omp end single
!!$omp parallel num_threads(CoreNumberOfCPU),shared(UG,CC),firstprivate(iCell)
!$omp do
    do iCell = 1, UG%GI%RealCells
