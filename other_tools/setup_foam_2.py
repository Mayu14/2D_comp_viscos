...
class Config(object):
    ...
    # RAS = "kEpsilon"  # "SA", "laminar"
    # k-epsilon model
    c_mu = 0.09
    turb_intensity = 0.01
    turb_characteristic_length = 0.1
    
    def __init__(self, reynolds, mach, shape, angle, env = "server", RAS="laminar", app="rhoSimpleFoam"):
        self.RAS = RAS
        self.app = app
    ...
    def load_default(self):
        ...
        self.remote_dir = '/work/A/FMa/FMa037/openFoam_invMk2'
        if self.env == "local":
        ...
        elif self.app == "rhoCentralFoam":
            self.endTime = 300.0
            self.deltaT = 0.00000001
            self.maxDeltaT = 1.0
            self.maxCourant = 0.85
            self.writeInterval = 5.0
            self.writeIntervalForces = 25
        else:
            raise ValueError
    ...
    def __gen_0(self, object = "U"):
        def gen_0_body(inletVariable, inletValue, dimension, inlet_fv, freestream_fv, wall_fv):
            inlet_field, inlet_value = inlet_fv[0], inlet_fv[1]
            freestream_field, freestream_value = freestream_fv[0], freestream_fv[1]
            wall_field, wall_value = wall_fv[0], wall_fv[1]
            boundary_field = ['inlet', 'freestream', 'wall', '#includeEtc']
            boundary_value = [FoamDict(inlet_field, inlet_value, depth = 2),
                              FoamDict(freestream_field, freestream_value, depth = 2),
        ...
        if object == 'U':
            ...
            inlet_field = ['type', "value"]
            inlet_value = ["fixedValue", "uniform $" + (inletVariable)]
            freestream_field = ['type']
            freestream_value = ['zeroGradient']
            wall_field = ['type']
            if self.reynolds == "inviscid":
                wall_value = ['slip']
            else:
                wall_value = ['noSlip']
        elif object == "T":
            class_type = 'volScalarField'
            inletVariable = 'Tinlet'
            inletValue = str(1.0)
            ...
            freestream_value = ['zeroGradient']
            wall_field = ['type']
            wall_value = ['zeroGradient']
        elif object == "p":
            class_type = 'volScalarField'
            inletVariable = 'pInlet'
            inletValue = str(1.0)
            dimensions = '[1 -1 -2 0 0 0 0]'
            if not self.incomp and self.mach > 1.0:
                inlet_field = ['type', "value"]
                inlet_value = ["fixedValue", "$internalField"]
            else:
                inlet_field = ['type']
                inlet_value = ["zeroGradient"]
            freestream_field = ['type']
            freestream_value = ['zeroGradient']
            wall_field = ['type']
            wall_value = ['zeroGradient']
            ...
            inlet_field = ['type', "freestreamValue"]
            inlet_value = ["freestream", "$internalField"]
            freestream_field = ['type', "freestreamValue"]
            freestream_value = ["freestream", "$internalField"]
            wall_field = ['type', 'value']
            wall_value = ['fixedValue', 'uniform 0']
        elif object == 'nut':
            class_type = 'volScalarField'
            inletVariable = 'nut'
            inletValue = self.__get_internalField_value(0.210428 * self.kinematic_viscosity)
            dimensions = '[0 2 -1 0 0 0 0]'
            inlet_field = ['type', "freestreamValue"]
            inlet_value = ["freestream", "$internalField"]
            freestream_field =  ['type', "freestreamValue"]
            freestream_value = ["freestream", "$internalField"]
            wall_field = ['type', 'value']
            wall_value = ['nutUSpaldingWallFunction', 'uniform 0']
        elif object == "k":
            class_type = "volScalarField"
            inletVariable = "kIn"
            ...
            exit()
        inlet_fv = [inlet_field, inlet_value]
        freestream_fv = [freestream_field, freestream_value]
        wall_fv = [wall_field, wall_value]
        zero = self.__gen_header(class_type, object)
        zero += gen_0_body(inletVariable, inletValue, dimensions, inlet_fv, freestream_fv, wall_fv)
    ...
    def __gen_constant(self, object = 'thermophysicalProperties'):
        ...
        elif object == "turbulenceProperties":
            if self.RAS != "laminar":
                RAS_value = FoamDict(["RASModel", "turbulence", "printCoeffs"], [self.RAS, "on", "on"])
                field_0 = ['simulationType', 'RAS']
                value_0 = ['RAS', RAS_value]
            else:
                field_0 = ["simulationType"]
                value_0 = [self.RAS]
        else:
    ...
    def __gen_system(self, object = 'controlDict'):
        ...
        if object == 'controlDict':
            ...
            value = [self.app, 'latestTime', str(self.startTime), 'endTime', str(self.endTime),
                     str(self.deltaT), str(self.maxDeltaT), 'runTime', str(self.writeInterval),
    ...
    def gen_p(self):
        return self.__save_text(generator = self.__gen_0, obj_name = 'p', dir = self.dir_0)
    
    def gen_T(self):
        return self.__save_text(generator = self.__gen_0, obj_name = 'T', dir = self.dir_0)
    ...
    def gen_turbulencePropaties(self):
        return self.__save_text(generator = self.__gen_constant, obj_name = 'turbulenceProperties',
                                dir = self.dir_const)
    ...
    def copy_all(self):
        def get_src_and_dst(dir, name):
            ...
            if ((name == "p") or ("fv" in name)):
                if self.incomp:
                    in_name += "_incomp"
            if (name == "fvSchemes") and (self.app == "rhoCentralFoam"):
                in_name += "_rcf"
...
def check_finish():
    ...
    gen = case_generator(alphabetical = True, case = "invMk2")
    while True:
        try:
            case_name = gen.__next__()  # 豈主屓蝗槭☆
    ...
    for case in completes:
        while True:
            n += 1
            if (n < total) and (not expected[n] == case):
...
def main_remake(naca4 = True):
    ...
    reynoldsList, machList, angleList, i1_list, i2_list, i3_list, i2_len = load_patternList(naca4, case = "invMk2")
    for mach in machList:
        for reynolds in reynoldsList:
            for angle in angleList:
                    ...
                    for i2 in i2_list:
                        if ((i1 == 0) and (i2 == 0)) or (i1 != 0):
                            for i34 in i3_list:
                                ...
                                con = Config(reynolds, mach, shape, angle, app = "rhoCentralFoam")
                                # print(con.casename)
                                # exit()
                                con.gen_all()
                                # con.gen_controlDict()
