...
!***********************************/
subroutine UGetLimiter(UConf,UG,UCC,UCE)
    use StructVar_Mod
    use LoopVar_Mod
...
!call FindVariation!(Geom,CC)
    ...
    if(UConf%KindLimiter == 1) then !Venkatakrishnan Limiter
        ...
        call UFindVariation(UG,UCC,PVOUV%UFV)
        call UGetNormalGradient(UG,UCC,UCE,PVOUV%UGNG)
        call UVenkatakrishnan(UG,UCC,UCE,PVOUV%UV)
    end if

return
end subroutine UGetLimiter
